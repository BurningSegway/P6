...
class ReachingFranka(gym.Env):
    def __init__(self, robot_ip="172.16.0.2", device="cuda:0", control_space="blind_agent", motion_type="waypoint", camera_tracking=False):
        ...
        self.observation_space_2 = gym.spaces.Box(low=-1000, high=1000, shape=(19,), dtype=np.float32)
        self.observation_space =  gym.spaces.Dict({"joint_pos": gym.spaces.Box(low=-1000, high=1000, shape=(9,), dtype=np.float32), 
                               "joint_vel": gym.spaces.Box(low=-1000, high=1000, shape=(7,), dtype=np.float32),
                                "object_position": gym.spaces.Box(low=-1000, high=1000, shape=(3,), dtype=np.float32)})
    ...
    def _get_observation_reward_done(self):
        ...
        joint_vel[0:7] = np.array(robot_state.dq)
        
        # Object position
        object_position = self.target_pos.astype(np.float32)
    ...
    def reset(self):
        ...
        if self.motion_type == "waypoint":
            ...
            self.motion = frankx.ImpedanceMotion(5, 1)
        else:
    ...
    def step(self, action):
        ...
        if self.control_space == "blind_agent":
            ...
            #dof_pos = robot_state.q + action[0:7]
            print(robot_state.q)
            print(action[0:7])
            print("dof_pos")
            print(dof_pos)
       ...
       #Gripper action goes here
        ...
        data_row.append(reward)         # Convert reward to float
        ...
        # Save with headers gem csv
        np.savetxt(f"test.csv", data_array, delimiter=",")
