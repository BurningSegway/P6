...
class ReachingFranka(gym.Env):
    ...
    def _get_observation_reward_done(self):
        ...
        self.joint_pos[0:7] = np.array(robot_state.q[0:7])
        self.joint_pos[7:8] = gripper_width
        self.joint_pos[8:9] = gripper_width

        self.joint_vel = np.zeros((7,))
        self.joint_vel[0:7] = np.array(robot_state.dq[0:7])
            ...
            while True:
                try:
                    ...
                    if raw:
                        ...
                        #self.target_pos = np.array([0.35, -0.17, 0.0])
                        #self.target_pos = np.array([0.35, -0.34, 0.0]) #Joint violations på alle
                        #Række 4
                        #self.target_pos = np.array([0.24, 0.34, 0.0])
                        #self.target_pos = np.array([0.24, -0.34, 0.0]) #Joint violations
                        #Række 5 
                        ...
                        #Within area
                        #Række 1
                        self.target_pos = np.array([0.55, 0.25, 0.0])
                        #self.target_pos = np.array([0.55, 0.08, 0.0])
                        #self.target_pos = np.array([0.55, -0.08, 0.0])
                        #self.target_pos = np.array([0.55, -0.25, 0.0])
                        #Række 2
                        #self.target_pos = np.array([0.50, 0.25, 0.0])
                        #self.target_pos = np.array([0.50, 0.08, 0.0])
                        #self.target_pos = np.array([0.50, -0.08, 0.0])
                        #self.target_pos = np.array([0.50, -0.25, 0.0])
                        #Række 3
                        #self.target_pos = np.array([0.45, 0.25, 0.0])
                        #self.target_pos = np.array([0.45, 0.08, 0.0])
                        #self.target_pos = np.array([0.45, -0.08, 0.0])
                        #self.target_pos = np.array([0.45, -0.25, 0.0])
                        #Række 4
    ...
    def step(self, action):
        ...
        if self.control_space == "joint":
            ...
            try:
                ...
                robot_state = self.robot.get_state(read_once=False)
            # forward kinematics
            dof_pos = np.array(robot_state.q[0:7]) + (self.robot_dof_speed_scales * self.dt * action[0:7] * self.action_scale)
            #dof_pos = np.clip(dof_pos, self.robot_dof_lower_limits, self.robot_dof_upper_limits)# i attemp to encorce constraints on joints
        ...
        #gripper_width = self.gripper.width()
        #gripper_width_target = (translate_a_to_c(action[7:8], -20, 10, 0.00, 0.08))
        ...
        #print("Gripper action is:", action[7:8])
